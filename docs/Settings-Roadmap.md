...
- 第一阶段已实现 DNS 三档策略、DoH/DoT 手动服务器、解析策略、缓存、独立缓存、FakeIP 和客户端子网。
- 第一阶段已实现 TUN MTU、协议栈、自动/严格路由、嗅探、覆盖目标、接口地址和排除路由。
- 高级覆写采用“订阅原配置 + 本地补丁”生成 `using_config`；保存前通过应用原生桥接调用 `Libbox.checkConfig`，校验成功后使用临时文件和备份文件替换，设置值最后持久化。
- 第二阶段已实现结构化路由覆写：默认出口、自动探测接口、双栈/仅 IPv4/仅 IPv6、常用规则，以及域名/IP/端口/进程/包名/Wi-Fi/网络类型自定义规则。
- 路由规则支持启停、编辑、删除和排序；写入前校验规则值、顺序、规则集和 outbound 引用。每次补丁从订阅原配置重建，避免重复保存累积规则。
- DNS、TUN 与路由均可单项恢复默认；默认继续使用订阅配置，不改变升级前行为。连接中保存会重载 VPN，未连接时在下次连接生效。
- 待实现：复杂 DNS 规则与响应规则、自动选优间隔/容差、规则集更新周期、日志与缓存、更新通道和内核维护。
...
| 订阅与规则 | 更新间隔、仅 Wi-Fi、失败重试、测速地址、订阅 User-Agent（预设/自定义列表，按订阅绑定，已实现）、自动选优间隔/容差、规则集来源/更新时间/下载出口 | T1-T2 |
